class synapse {
  ...
  void conveydesired() {
    net.layers.get(round(start.x)).neurons.get(round(start.y)).desiredact+=(net.layers.get(round(destination.x)).neurons.get(round(destination.y)).desiredact-net.layers.get(round(destination.x)).neurons.get(round(destination.y)).activation)*amplifier;
    //net.layers.get(round(start.x)).neurons.get(round(start.y)).desiredact+=net.layers.get(round(destination.x)).neurons.get(round(destination.y)).gradient*amplifier;
}
